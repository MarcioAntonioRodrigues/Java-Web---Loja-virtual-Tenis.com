...
<nav class="navbar navbar-expand-lg navbar-light bg-light">
  ...
  <form class="form-inline my-2 my-lg-0" method="post" action="BuscarController">
      <input style="width: 500px;" class="form-control mr-sm-2" type="search" placeholder="Buscar" aria-label="Search" name="procurar">
...
<!-- Fim do Menu -->
  ...
  <c:if test="${produto.getNome().toLowerCase().contains(nomeProduto.toLowerCase())}">  
     <c:set var="encontrou" value="true"/>
		<div class="vitrine">
			<form action="CarrinhoController" method="post">
				<img style="width:300px;" src="${produto.getImagem()}"/>
				<P>${produto.getNome()}</p>
				<p>R$ ${produto.getPreco()}</p>
				<input type="hidden" name="Comprar" value ="${produto.getCodproduto()}"/>
    ...
    <br/>
  <br/>
 	<h1 style="clear:both;text-align:center;font-size:25px;">Desculpe, o produto não foi encontrado</h1>
 	 </c:if>
 	  <br/>
 	   <br/>
 	
 	<p class="buttonLink">
 	  <a href="CategoriaController">Voltar
 	  </a>
 	</p>
